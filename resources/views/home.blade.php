        ...
        <div class="max-w-6xl mx-auto px-4">
            <div class="text-center text-white">
                <h1 class="text-3xl md:text-4xl font-bold mb-4">
                    Rumahnya Artikel Ilmiah Indonesia
                </h1>
                <p class="text-lg md:text-xl mb-6 text-blue-100">
                    Portal publikasi dan diskusi ilmiah untuk membantu karya Anda lebih mudah ditemukan, dibaca, dan
                    dikutip.
                </p>
    ...
    <div class="bg-white py-8 md:py-12">
        <div class="max-w-6xl mx-auto px-4">
            ...
            <div class="flex gap-4 mb-6 overflow-x-auto pb-2">
                ...
                @endphp
                <a href="/"
                    class="px-4 py-2 bg-gray-200 text-gray-700 rounded-full whitespace-nowrap hover:bg-gray-300 transition">
                    Semua
                </a>
                @foreach($categories as $cat)
                    <a href="/artikel?category={{ $cat }}"
                        class="px-4 py-2 bg-gray-100 text-gray-700 rounded-full whitespace-nowrap hover:bg-gray-200 transition">
                        {{ $cat }}
                    </a>
                @endforeach
            </div>
                            ...
                            <div class="bg-white rounded-lg overflow-hidden hover:shadow-xl transition h-full flex flex-col">
                                ...
                                <div class="relative flex-shrink-0">
                                    @if($featuredArticle->image_path)
                                        <img src="{{ asset('storage/' . $featuredArticle->image_path) }}"
                                            alt="{{ $featuredArticle->title }}" class="w-full h-64 lg:h-80 object-cover">
                                    @else
                                        <div
                                            class="w-full h-64 lg:h-80 bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center">
                                    ...
                                    @endphp
                                    <span
                                        class="absolute top-4 left-4 px-3 py-1 {{ $bgColor }} text-white text-sm font-bold rounded-full shadow-lg">
                                ...
                                <div class="p-6 flex-grow flex flex-col">
                                    <h3
                                        class="text-xl lg:text-2xl font-bold text-gray-900 group-hover:text-blue-600 line-clamp-2 mb-3 leading-tight">
                                    ...
                                    <div class="space-y-2 mb-3">
                                        @if($featuredArticle->reviewer_name)
                                            <div class="flex items-center text-xs text-gray-600">
                                                ...
                                                <span><strong>Ditinjau oleh:</strong>
                                                    {{ $featuredArticle->reviewer_name }}@if($featuredArticle->reviewer_title),
                                                    {{ $featuredArticle->reviewer_title }}@endif</span>
                                            </div>
                                        @endif
                                        <div class="flex items-center text-xs text-gray-600">
                                            ...
                                            <span><strong>Diperbarui:</strong>
                                                {{ $featuredArticle->updated_at->diffForHumans() }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div
                                        ...
                                        <span
                                            class="font-medium">{{ $featuredArticle->author_institution ?? 'Scholar System' }}</span>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 h-full">
                        @foreach($articles as $article)
                            <a href="/artikel/{{ $article->slug }}" class="block group">
                                <div
                                    class="bg-white rounded-lg overflow-hidden hover:shadow-xl transition h-full flex flex-col">
                                    <!-- Image -->
                                    <div class="relative flex-shrink-0">
                                        ...
                                        @else
                                            <div
                                                class="w-full h-40 lg:h-44 bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center">
                                        ...
                                        @endphp
                                        <span
                                            class="absolute top-3 left-3 px-2.5 py-1 {{ $bgColor }} text-white text-xs font-bold rounded-full shadow-md">
                                    ...
                                    <div class="p-4 flex-grow flex flex-col">
                                        <h3
                                            class="text-sm lg:text-base font-bold text-gray-900 group-hover:text-blue-600 line-clamp-2 mb-2 leading-snug">
                                            {{ $article->title }}
                                        </h3>
                                        ...
                                        @if($article->reviewer_name)
                                            <div class="flex items-center text-xs text-gray-600 mb-2">
                                                <svg class="w-3 h-3 mr-1 text-green-600 flex-shrink-0" fill="currentColor"
                                                    viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd"
                                                        d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                                                        clip-rule="evenodd" />
                                                </svg>
                                                <span class="truncate">Ditinjau oleh
                                                    {{ Str::limit($article->reviewer_name, 20) }}</span>
                                            </div>
                                        @endif

                                        <div
                                            class="flex items-center justify-between text-xs text-gray-500 pt-2 border-t border-gray-100 mt-auto">
                                            <span class="truncate">{{ $article->author_institution ?? 'Scholar System' }}</span>
                                            <span
                                                class="text-xs whitespace-nowrap ml-2">{{ $article->updated_at->diffForHumans() }}</span>
                                        </div>
                                    </div>
                                </div>
                            </a>
        ...
        <div class="max-w-6xl mx-auto px-4">
            ...
            <div class="grid md:grid-cols-3 gap-8">
                <div class="text-center">
                    ...
                    <p class="text-sm text-gray-600">Semua artikel ditinjau oleh dosen dan peneliti berpengalaman dari
                        universitas terkemuka</p>
                </div>
                <div class="text-center">
                    ...
                    <p class="text-sm text-gray-600">Konten kami diperbarui secara berkala mengikuti perkembangan penelitian
                        terbaru</p>
                </div>
                <div class="text-center">
                    ...
                    <p class="text-sm text-gray-600">Artikel kami terindeks di Google Scholar dan dapat disitasi untuk
                        penelitian akademik</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="max-w-6xl mx-auto px-4">
            <div class="grid md:grid-cols-2 gap-8">
                ...
                <div class="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
                    ...
                    <a href="/login"
                        class="block w-full text-center px-6 py-3 border-2 border-blue-600 text-blue-600 rounded-lg font-semibold hover:bg-blue-50 transition">
                        Mulai Publikasi
                    </a>
                </div>
            </div>
